...
class ExpensesController extends Controller
{
        ...
        if ($data['credit_card']) {
            if ($installmentsExpense > 1) {
                ...
                for ($i = 1; $i <= $installmentsExpense; $i++) {
                    ...
                    if ($invoice) {
                        CreditCardInvoiceModel::where('id', $invoice->id)->update([
                        ...
                        ExpenseInstallmentsModel::create(['expense' => $expense->id, 'hash_installment' => $uniqid]);
                        $this->month++;
                    } else {
            ...
            return response()->json(['ok' => true, 'message' => 'Saída cadastrada com sucesso no cartão de crédito.']);
        }
    ...
    public function destroyAll($id)
    {
        ...
        return response()->json(['ok' => true, 'message' => 'O parcelamento foi excluído com sucesso.']);
    }

    public function revert($id)
    {
        $expense = ExpenseModel::find($id);
        ExpenseModel::where('id', $expense['id'])->where('user_id', session('user')['id'])->update([
            'cancelled' => 0,
            'reason_cancelled' => null
        ]);

        return response()->json(['ok' => true, 'msg' => 'Parcela revertida com sucesso!']);
    }

    public function cancelOneInstallment()
    {
        $data = request()->all();
        ExpenseModel::where('id', $data['id_expense'])->update([
            'cancelled' => 1,
            'reason_cancelled' => $data['reason_cancelled'],
            'realized_amount' => 0
        ]);

        return response()->json(['ok' => true, 'message' => 'Saída atualizada com sucesso']);
    }

    public function cancelAllInstallments()
    {
        $data = request()->all();
        $cancelInstallments = $this->expenseModel->cancelExpenses($data['id_expense'], $data['reason_cancelled_all']);
        if ($cancelInstallments['ok']) {
            return response()->json(['ok' => true, 'message' => $cancelInstallments['msg']]);
        }

        return response()->json(['ok' => false, 'message' => 'Erro interno ao cancelar parcelas. Verifique com o suporte']);
    }
    ...
    public function update()
    {
        ...
        if ($data['credit_card_edit'] != null) {
            ...
            if (!$invoice) {
                ...
                if (floatval($data['budgeted_amount_expense_edit']) == floatval($data['realized_amount_expense_edit']) && $expensesPaid == count($expenses)) {
                    CreditCardInvoiceModel::where('id', $invoice->id)->update([
                        'credit_card' => $data['credit_card_edit'],
                        'payment' => 1,
        ...
        if ($data['payment_method_change'] == 'N') {
            ...
            if ($expense->installments > 1) {
                $expenseInstallment = ExpenseInstallmentsModel::where('expense', $data['id_expense'])->select('hash_installment')->first();
                $expensesWithInstallments = ExpenseInstallmentsModel::where('hash_installment', $expenseInstallment['hash_installment'])->select('expense')->get();
                foreach ($expensesWithInstallments as $expenseDetail) {
                    if ($data['category_active'] == 1) {
                        ExpenseModel::where('id', $expenseDetail['expense'])->update([
                            'description' => mb_convert_case($data['description_expense_edit'], MB_CASE_TITLE, "UTF-8"),
                            'category' => $data['category_expense_edit'],
                            'period' => $data['period_expense_edit'],
                            'budgeted_amount' => $data['budgeted_amount_expense_edit'],
    ...
    public function verifyMetodPayment()
    {
        ...
        $ok = true;
        $message = 'Forma de pagamento permanece a mesma';
        $expense = ExpenseModel::find($data['id_expense']);
        if ($expense->credit_card != $data['credit_card_edit'] && $expense->installments > 1) {
            $ok = false;
            $message = 'Alterou a forma de pagamento';
        }
    ...
    public function extendInstallments($id)
    {
        ...
        foreach ($expensesToExtend as $expense) {
            ...
            ExpenseModel::where('id', $expense->expense)->update([
                'month' => $month,
                'year' => $year
            ]);
        }

        return response()->json(['ok' => true, 'message' => 'As saídas foram prorrogadas com sucesso.']);
    }
    ...
    public function pay($id)
    {
        ...
        return response()->json(['ok' => true, 'message' => 'Saída paga com sucesso.']);
    }
}
